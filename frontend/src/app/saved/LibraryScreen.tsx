...
export default function LibraryScreen() {
  ...
  const [popupToastAnim] = useState(() => new Animated.Value(0));

  const showPopupToast = (toast: {
    message: string;
    actionLabel?: string;
    onAction?: () => void;
  }) => {
    setPopupToast(toast);
    popupToastAnim.setValue(0);
    Animated.timing(popupToastAnim, { toValue: 1, duration: 280, useNativeDriver: true }).start();
  };
  ...
  return (
    <View style={{ flex: 1, backgroundColor: "#F2F3F5" }}>
      ...
      >
        ...
        <Pressable
          onPress={() => router.push("/saved/BookmarksScreen")}
          style={{
        ...
        <View style={{ flexDirection: "row" }}>
          <Pressable
            onPress={() => router.push("/saved/DownloadsScreen")}
            style={{
              flex: 1,
              marginRight: 12,
              backgroundColor: "#153A7A",
          ...
          <Pressable
            onPress={() => router.push("/saved/HistoryScreen")}
            style={{
              flex: 1,
              backgroundColor: "#153A7A",
              padding: 12,
        ...
        {playlists.length === 0 ? (
          ...
          playlists.map((playlist) => (
            <Pressable
              key={playlist.id}
              onPress={() => router.push(`/saved/PlaylistScreen?playlistId=${playlist.id}`)}
              style={{
        ...
        >
          ...
          {playlists.length === 0 ? (
            <View style={{ paddingVertical: 35, justifyContent: "center", alignItems: "center" }}>
              <Text style={[Typography.bodyMd, { color: "#909090" }]}>
            ...
            <ScrollView style={{ maxHeight: 300 }}>
              {playlists.map((item) => {
                const selected = item.id === libraryPopupCreatedId;
                return (
                  <Pressable
                    key={item.id}
                    onPress={() => {
                      closeLibraryPopup();
                      setTimeout(
                        () => router.push(`/saved/PlaylistScreen?playlistId=${item.id}`),
                        300,
                      );
                    }}
                    style={{
                  ...
                  >
                    ...
                    <View style={{ flex: 1, gap: 8 }}>
                      ...
                      <Text style={[Typography.caption, { color: "#153A7A" }]}>
                        {item.activityIds.length}{" "}
                        {item.activityIds.length === 1 ? "activity" : "activities"}
                      </Text>
                    </View>
                    {selected ? (
                      <View
