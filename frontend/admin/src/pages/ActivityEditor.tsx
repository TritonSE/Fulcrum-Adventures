...
const getSubmissionValidationMessage = ({
  ...
  if (!overviewValue.thumbnailImage) return "Please upload either an image or a video frame.";
      ...
      const prefix =
        kind === "play"
          ...
          : kind === "debrief"
            ? /^Q\d+:/i.exec(block)
            : null;
      const guidedText = prefix ? block.slice(prefix[0].length).trim() : "";
...
const createOverviewStateFromActivity = (activity: ActivityDetail): OverviewFormState => {
  ...
  const existingVideoUrl = typeof activity.videoUrl === "string" ? activity.videoUrl : "";
...
export const ActivityEditor: React.FC<ActivityEditorProps> = ({
  ...
  const validateForm = (tabsToValidate = activityTabs): ValidationResult => {
    ...
    if (!overviewValue.thumbnailImage) {
      newErrors.thumbnail = "Please upload either an image or a video frame";
    }
  ...
  const submitActivity = async (targetStatus: ApiActivityStatus) => {
    ...
    if (mode === "edit" && !activityIdParam) return null;
    ...
    try {
      if (mode === "create") {
        ...
        showToast(
          "success",
          (savedActivity.status ?? targetStatus) === "Published"
            ? "Activity published."
      ...
      showToast(
        ...
        error instanceof Error
          ...
          : mode === "edit"
            ...
            : "Unable to submit activity. Please try again.",
      );
      return null;
    } finally {
  ...
  if (loadError || (mode === "edit" && !activityIdParam)) {
    return (
      <View style={styles.centerState}>
